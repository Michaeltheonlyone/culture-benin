...
<head>
    <meta charset="UTF-8">
    <title>{{ $user->prenom }} {{ $user->nom }} | Portail Culturel du Bénin</title>
...
<div class="max-w-7xl mx-auto py-6 px-4">
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <!-- LEFT SIDEBAR - STICKY (Same as feed) -->
        ...
        <main class="lg:col-span-2 space-y-6 overflow-y-auto max-h-[calc(100vh-5rem)] pr-2">
            <!-- Profile Header -->
            <div class="bg-white rounded-2xl overflow-hidden border border-gray-200 card-shadow">
                <!-- Cover Photo -->
                <div class="h-48 bg-gradient-to-r from-blue-500 to-blue-600 relative overflow-hidden">
                    @if($user->header)
                        @php
                            $headerUrl = null;
                            if (function_exists('Storage') && method_exists('Storage', 'url')) {
                                try {
                                    $headerUrl = Storage::url($user->header);
                                } catch (\Exception $e) {}
                            }
                            if (!$headerUrl || !filter_var($headerUrl, FILTER_VALIDATE_URL)) {
                                $headerUrl = asset('storage/' . $user->header);
                            }
                            if (!$headerUrl || !filter_var($headerUrl, FILTER_VALIDATE_URL)) {
                                $headerUrl = asset('headers/default.jpeg');
                            }
                        @endphp
                        <img src="{{ $headerUrl }}"
                             class="w-full h-full object-cover"
                             alt="Bannière de {{ $user->prenom }}"
                             onerror="this.onerror=null; this.src='{{ asset('headers/default.jpeg') }}';">
                    @endif
                    ...
                    <div class="absolute -bottom-16 left-6">
                        @php
                            ...
                            if (!$photoUrl || !filter_var($photoUrl, FILTER_VALIDATE_URL)) {
                                $photoUrl = asset('photos/default.jpeg');
                            }
                        @endphp
                        <img src="{{ $photoUrl }}"
                             class="w-32 h-32 rounded-full border-4 border-white object-cover shadow-lg"
                             alt="Photo de {{ $user->prenom }}"
                             onerror="this.onerror=null; this.src='{{ asset('photos/default.jpeg') }}';">
                    </div>
                    
                    <!-- Back Button -->
                    <a href="{{ route('frontend.contenus.feed') }}" 
                       class="absolute top-4 left-4 w-10 h-10 bg-white/90 hover:bg-white rounded-full flex items-center justify-center shadow transition">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-gray-600 mt-1">
                                @<span class="font-medium">{{ \Illuminate\Support\Str::slug($user->prenom . $user->nom) }}</span>
                            </p>
                            ...
                            <div class="flex flex-wrap gap-3 mt-4">
                                @if($user->langue)
                                    <div class="flex items-center gap-2 px-3 py-1.5 bg-blue-50 rounded-full">
                                        ...
                                        <span class="text-sm font-medium text-blue-700">{{ $user->langue->nom_langue }}</span>
                                    </div>
                                @endif
                                
                                <div class="flex items-center gap-2 px-3 py-1.5 bg-amber-50 rounded-full">
                                    <i class="fas fa-star text-amber-600"></i>
                                    <span class="text-sm font-medium text-amber-700">{{ $posts->count() }} contenus</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-100">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="text-sm text-gray-500">Dernière activité</div>
                                    <div class="font-medium">{{ $user->updated_at->diffForHumans() }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="flex items-center justify-between mb-4">
                    <h2 class="text-xl font-bold text-gray-900 flex items-center gap-2">
                        ...
                        Contenus publiés
                        <span class="text-sm font-normal text-gray-500">({{ $posts->count() }})</span>
                    </h2>
                </div>

                <!-- User's Posts - 280 chars limit -->
                    ...
                    <div class="bg-white rounded-xl p-5 border border-gray-200 card-shadow">
                        ...
                        <a href="{{ route('contenus.show', UrlEncrypter::encrypt($post->id)) }}" class="block group">
                            ...
                            <div class="text-gray-700 mb-4 whitespace-pre-line leading-relaxed">
                                {{ \Illuminate\Support\Str::limit($post->contenu, 280) }}
                                @if(strlen($post->contenu) > 280)
                        ...
                        @if ($post->medias->count())
                            <div class="mb-4 rounded-xl overflow-hidden border border-gray-200">
                                @foreach ($post->medias->take(1) as $media)
                                    @php
                                        $mediaUrl = null;
                                        ...
                                        if (!$mediaUrl || !filter_var($mediaUrl, FILTER_VALIDATE_URL)) {
                                            $mediaUrl = asset('assets/images/default.jpg');
                                        }
                                    @endphp
                                                ...
                                                <a href="{{ $mediaUrl }}" target="_blank"
                                                   class="flex items-center gap-3 text-blue-600 hover:text-blue-800">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="text-sm text-gray-600">Cliquez pour ouvrir</div>
                                                    </div>
                                                </a>
                                            </div>
                                            @break
                        ...
                        <div class="mb-4">
                            <span class="px-3 py-1.5 rounded-full bg-gray-100 text-gray-700 text-sm font-medium">
                                ...
                                {{ $post->typeContenu->nom ?? 'Contenu' }}
                            </span>
                        </div>

                        <!-- Actions - Only Comment and Share -->
                        <div class="flex justify-between items-center pt-4 border-t border-gray-100">
                            <div class="flex gap-6">
                                ...
                                <form action="{{ route('contenus.storeFront') }}" method="POST" class="inline">
                                    ...
                                    <input type="hidden" name="contenu" value="{{ $post->contenu }}">
                                    <input type="hidden" name="id_region" value="{{ $post->id_region }}">
                                    <input type="hidden" name="id_type_media" value="{{ $post->id_type_media }}">
                                    <input type="hidden" name="id_type_contenu" value="{{ $post->id_type_contenu }}">
                                    <button type="submit"
                            ...
                            @if($post->langue)
                                <div class="text-sm text-gray-500 flex items-center gap-1">
                                    ...
                                    <span>{{ $post->langue->nom_langue }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                @empty
                    <div class="bg-white rounded-xl p-8 border border-gray-200 text-center">
                        ...
                        <h3 class="text-lg font-bold text-gray-900 mb-2">Aucune publication</h3>
                        <p class="text-gray-600 mb-4">{{ $user->prenom }} n'a pas encore partagé de contenu.</p>
                        <a href="{{ route('frontend.contenus.feed') }}" 
                           class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-lg font-medium transition">
        ...
        <!-- RIGHT SIDEBAR - STICKY (Same as feed) -->
        <aside class="lg:col-span-1 space-y-6 sticky top-20 h-[calc(100vh-5rem)] overflow-y-auto">
            <!-- Search -->
            <div class="bg-white rounded-xl p-5 border border-gray-200 card-shadow">
                               ...
                               class="flex items-center gap-3 p-3 rounded-lg hover:bg-blue-50 transition duration-300 group">
                                ...
                                <div class="flex-1">
                                    <div class="font-semibold text-gray-900 group-hover:text-blue-700 transition duration-300">
                                        {{ \Illuminate\Support\Str::limit($puser->name ?? ($puser->prenom.' '.$puser->nom), 15) }}
                                    </div>
                                    <div class="text-xs text-gray-600">{{ $puser->contenus_count ?? 0 }} contenus</div>
                                </div>
                            </a>
                        @endif
                    @empty
